...
public class ElizonPermsVelocityCommand implements SimpleCommand {
    ...
    public void execute(SimpleCommand.Invocation invocation) {
        ...
        if (!sender.hasPermission("elizonperms.command.execute")) {
            sender.sendMessage(Component.text("§f[§9EP§f] §aThis server is using the free ElizonPerms Lite permissions system by ELIZONMedia. Thanks for using!"));
            return;
        }

        if (args.length < 3) {
            player.sendMessage(Component.text("§f[§9EP§f] §9Usage for groups:\n" +
                    "  /epv group <name> info\n" +
                    "  /epv group <name> permission info\n" +
                    "  /epv group <name> create\n" +
                    "  /epv group <name> delete\n" +
                    "  /epv group <name> rename <new name>\n" +
                    "  /epv group <name> clone <name>\n" +
                    "  /epv group <name> permission add <permission>\n" +
                    "  /epv group <name> permission remove <permission>\n" +
                    "  /epv group <name> permission set <permission> true\n" +
                    "  /epv group <name> permission set <permission> false\n" +
                    "  /epv group <name> permission set <permission> data (Initialize data via API)\n" +
                    "  /epv group <name> inherit info\n" +
                    "  /epv group <name> inherit add <group to inherit>\n" +
                    "  /epv group <name> inherit remove <group>\n" +
                    "  /epv group <name> setdefault\n" +
                    "  /epv group <name> setprefix <prefix>\n" +
                    "  /epv group <name> setsuffix <suffix>\n" +
                    "  /epv group <name> setheight <height>\n" +
                    "§f[§9EP§f] §9Usage for users:\n" +
                    "  /epv user <name> info\n" +
                    "  /epv user <name> permission info\n" +
                    "  /epv user <name> group info\n" +
                    "  /epv user <name> group add <group>\n" +
                    "  /epv user <name> group addtimed <group> <time in hours>\n" +
                    "  /epv user <name> group set <group> true\n" +
                    "  /epv user <name> group set <group> false\n" +
                    "  /epv user <name> group set <group> data (Initialize data via API)\n" +
                    "  /epv user <name> group settimed <group> true <time in hours>\n" +
                    "  /epv user <name> group settimed <group> false <time in hours>\n" +
                    "  /epv user <name> group settimed <group> data <time in hours> (Initialize data via API)\n" +
                    "  /epv user <name> permission add <permission>\n" +
                    "  /epv user <name> permission remove <permission>\n" +
                    "  /epv user <name> permission set <permission> true\n" +
                    "  /epv user <name> permission set <permission> false\n" +
                    "  /epv user <name> permission set <permission> data (Initialize data via API)\n" +
                    "§f[§9EP§f] §9Usage for tracks:\n" +
                    "  /epv track <name> list\n" +
                    "  /epv track <name> delete\n" +
                    "  /epv track <name> create <group> <group> <group>...\n" +
                    "  /epv track <name> insertbefore <group to insert> <next higher group>\n" +
                    "  /epv track <name> add <new highest group>\n" +
                    "  /epv track <name> remove <group>\n" +
                    "  /epv track <name> next <group>\n" +
                    "  /epv track <name> rankup <username>"));
    ...
    // Import required packages and classes if necessary

    private void handleTrackAction(CommandSource player, String targetName, String actionType, String[] args) {
        PermGroupTrack groupTrack = new PermGroupTrack();

        if (!player.hasPermission("elizonperms.track." + actionType.toLowerCase())) {
            player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to perform this action on tracks."));
            return;
        }
        ...
        switch (actionType.toLowerCase()) {
            case "insertbefore":
                ...
                if (args.length < 5) {
                    player.sendMessage(Component.text("§f[§9EP§f] §cMissing arguments."));
                    return;
                }
                String groupToInsert = args[3];
                String nextGroup = args[4];
                groupTrack.insertGroupBeforeNext(targetName, groupToInsert, nextGroup);
                player.sendMessage(Component.text("§f[§9EP§f] §aInserted group " + groupToInsert + " before " + nextGroup + " in track " + targetName));
                break;
            case "create":
                // Add group to end
                if (args.length < 3) {
                ...
                player.sendMessage(Component.text("§f[§9EP§f] §aCreated track " + targetName + " with the following groups: " + str));
                break;
            ...
            case "add":
                ...
                player.sendMessage(Component.text("§f[§9EP§f] §aAdded group " + groupToAdd + " to the end of track " + targetName));
                break;
            ...
            case "remove":
                ...
                player.sendMessage(Component.text("§f[§9EP§f] §cRemoved group " + groupToRemove + " from the track " + targetName));
                break;

            case "delete":
                // Delete track
                groupTrack.deleteTrack(targetName);
                player.sendMessage(Component.text("§f[§9EP§f] §cDeleted track " + targetName));
                break;
            ...
            case "next":
                ...
                if (nextGroup2 != null) {
                    player.sendMessage(Component.text("§f[§9EP§f] §bNext group after " + currentGroup + " in track " + targetName + ": " + nextGroup2));
                } else {
                    player.sendMessage(Component.text("§f[§9EP§f] §cNo next group found after " + currentGroup + " in track " + targetName));
                }
                break;
            ...
            case "list":
                ...
                if (groups != null && !groups.isEmpty()) {
                    player.sendMessage(Component.text("§f[§9EP§f] §bGroups in track " + targetName + ": " + String.join(", ", groups)));
                } else {
                    player.sendMessage(Component.text("§f[§9EP§f] §cNo groups found in track " + targetName));
                }
                break;
            ...
            case "rankup":
                ...
                if (!player.hasPermission("elizonperms.user.group.set")) {
                    player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to perform this action on groups."));
                    return;
                }
                ...
                if (playerName.startsWith("uuid:")) {
                    ...
                    if (proxy.getPlayer(playerName).isEmpty()) {
                        player.sendMessage(Component.text("§f[§9EP§f] §cThe target player is offline. Use the uuid like this instead: \"uuid:UUID HERE\""));
                        return;
                    }
                    target = new PermPlayer(proxy.getPlayer(playerName).get().getUniqueId().toString());
                }

                if (target.getGroups().size() != 1) {
                    player.sendMessage(Component.text("§f[§9EP§f] §cThere is no rank to rankup. Please note, that the player must have only one rank."));
                } else {
                    String next = groupTrack.getNextGroup(targetName, target.getGroups().getFirst());
                    if (next != null) {
                        target.setGroup(next, -1);
                        player.sendMessage(Component.text("§f[§9EP§f] §aRanked up player from group " + target.getGroups().getFirst() + " to " + next + "."));
                    } else {
                        player.sendMessage(Component.text("§f[§9EP§f] §cThere is no rank to rankup. Please note, that the player must have only one rank."));
                    }
                }
            ...
            default:
                player.sendMessage(Component.text("§f[§9EP§f] §cInvalid action type."));
                break;
        }
    }
    ...
    private void handleGroupAction(CommandSource player, String targetName, String actionType, String[] args) {
        ...
        if (actionType.equalsIgnoreCase("permission")) {
            if (args.length < 4) {
                player.sendMessage(Component.text("§f[§9EP§f] §cMissing argument."));
                return;
            }
            ...
            switch (action.toLowerCase()) {
                case "add":
                    ...
                    if (!player.hasPermission("elizonperms.group.permission.add")) {
                        player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to add permissions to groups."));
                        return;
                    }
                    ...
                    player.sendMessage(Component.text("§f[§9EP§f] §aAdding permission " + args[4] + " to group " + targetName));
                    break;
                case "remove":
                    // Permission check for remove action
                    if (!player.hasPermission("elizonperms.group.permission.remove")) {
                        player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to remove permissions from groups."));
                        return;
                    }
                    ...
                    player.sendMessage(Component.text("§f[§9EP§f] §cRemoving permission " + args[4] + " from group " + targetName));
                    break;
                case "set":
                    // Permission check for set action
                    if (!player.hasPermission("elizonperms.group.permission.set")) {
                        player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to set permissions for groups."));
                        return;
                    }
                    //value
                    if (args.length < 6) {
                    ...
                    player.sendMessage(Component.text("§f[§9EP§f] §aSetting permission " + args[4] + " (" + args[5] + ") for group " + targetName));
                    break;
                case "info":
                    // Permission check for info action
                    if (!player.hasPermission("elizonperms.group.permission.info")) {
                        player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to view permission info for groups."));
                        return;
                    }
                    player.sendMessage(Component.text("§f[§9EP§f] §bGetting info about permission in group " + targetName + "..."));
                ...
                default:
                    player.sendMessage(Component.text(
                            "§f[§9EP§f] §bCommand Help §f(/epv group " + targetName + " permission ... <string> <true|false|data>) \n" +
                                    "§f[§9EP§f] §9> §aadd\n" +
                                    "§f[§9EP§f] §9> §cremove\n" +
                                    "§f[§9EP§f] §9> §aset\n" +
                                    "§f[§9EP§f] §9> §ainfo"
            ...
            switch (action.toLowerCase()) {
                case "add":
                    ...
                    if (!player.hasPermission("elizonperms.group.inherit.add")) {
                        player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to add inherit groups to groups."));
                        return;
                    }
                    ...
                    player.sendMessage(Component.text("§f[§9EP§f] §aNow inheriting group " + args[4] + " at group " + targetName));
                    break;
                case "remove":
                    // Permission check for remove action
                    if (!player.hasPermission("elizonperms.group.inherit.remove")) {
                        player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to remove group inherits from groups."));
                        return;
                    }
                    ...
                    player.sendMessage(Component.text("§f[§9EP§f] §cRemoving inherit of group " + args[4] + " from group " + targetName));
                    break;
                case "info":
                    // Permission check for info action
                    if (!player.hasPermission("elizonperms.group.inherit.info")) {
                        player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to view inherit info for groups."));
                        return;
                    }
                    player.sendMessage(Component.text("§f[§9EP§f] §bGetting info about inherits of group " + targetName + "..."));
            ...
            if (!player.hasPermission("elizonperms.group.create")) {
                player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to create groups."));
                return;
            }

            group.setAsDefaultGroup();
            player.sendMessage(Component.text("§f[§9EP§f] §aGroup " + targetName + " is now the default group."));
            ...
            player.sendMessage(Component.text("§f[§9EP§f] §aGroup " + targetName + " ( " + group.build() + " ) created."));
        } else if (actionType.equalsIgnoreCase("delete")) {
            // Permission check for delete action
            if (!player.hasPermission("elizonperms.group.delete")) {
                player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to delete groups."));
                return;
            }
            group.deleteGroup();
            player.sendMessage(Component.text("§f[§9EP§f] §cGroup " + targetName + " deleted successfully."));
            ...
            if (!player.hasPermission("elizonperms.group.rename")) {
                player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to rename groups."));
                return;
            }
            if (args.length < 4) {
                player.sendMessage(Component.text("§f[§9EP§f] §cMissing argument."));
            ...
            player.sendMessage(Component.text("§f[§9EP§f] §bGroup " + targetName + " renamed to " + newName + "."));
        } else if (actionType.equalsIgnoreCase("info")) {
            // Permission check for info action
            if (!player.hasPermission("elizonperms.group.info")) {
                player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to view general info about groups."));
                return;
            }
            ...
            player.sendMessage(Component.text("§f[§9EP§f] §bGeneral Information for Group: " + targetName + "\n" +
            ...
            if (!player.hasPermission("elizonperms.group.clone")) {
                player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to clone groups."));
                return;
            }
            group.cloneGroup(args[3], false);
            player.sendMessage(Component.text("§f[§9EP§f] §aCloning group " + targetName + " to group " + args[3]));
        } else {
            // Help message
            player.sendMessage(Component.text(
                    "§f[§9EP§f] §bCommand Help §f(/epv group " + targetName + " <action>)\n" +
    ...
    private void handleUserAction(Player player, String targetName, String actionType, String[] args) {
        ...
        if(targetName.toLowerCase().startsWith("uuid:")) {
            ...
            if(proxy.getPlayer(targetName).isEmpty()) {
                player.sendMessage(Component.text("§f[§9EP§f] §cThe target player is offline. Use uuid: with the uuid of the target player instead."));
                return;
            }
            permPlayer = new PermPlayer(proxy.getPlayer(targetName).get().getUniqueId().toString());
        }

        if (!player.hasPermission("elizonperms.user." + actionType.toLowerCase())) {
            player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to perform this action on users."));
            return;
        }
        ...
        if (actionType.equalsIgnoreCase("group")) {
            ...
            switch (action.toLowerCase()) {
                case "add":
                    ...
                    if (!player.hasPermission("elizonperms.user.group.add")) {
                        player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to add users to groups."));
                        return;
                    }
                    if(args.length < 5) {
                        player.sendMessage(Component.text("§f[§9EP§f] §cMissing argument."));
                    ...
                    player.sendMessage(Component.text("§f[§9EP§f] §aAdding user " + targetName + " to group " + args[4]));
                    break;
                case "addtimed":
                    // Permission check for set action
                    if (!player.hasPermission("elizonperms.user.group.set")) {
                        player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to set user groups."));
                        return;
                    }
                    if(args.length < 6) {
                        player.sendMessage(Component.text("§f[§9EP§f] §cMissing argument."));
                    ...
                    player.sendMessage(Component.text("§f[§9EP§f] §aSetting group for user " + targetName + " to " + args[4]));
                    break;
                case "remove":
                    // Permission check for remove action
                    if (!player.hasPermission("elizonperms.user.group.remove")) {
                        player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to remove users from groups."));
                        return;
                    }
                    if(args.length < 5) {
                        player.sendMessage(Component.text("§f[§9EP§f] §cMissing argument."));
                    ...
                    player.sendMessage(Component.text("§f[§9EP§f] §cRemoving user " + targetName + " from group " + args[4]));
                    break;
                case "set":
                    // Permission check for set action
                    if (!player.hasPermission("elizonperms.user.group.set")) {
                ...
                case "info":
                    ...
                    if (!player.hasPermission("elizonperms.user.group.info")) {
                        player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to view group info for users."));
                        return;
                    }
                    List<String> groups = permPlayer.getGroups();
                    player.sendMessage(Component.text("§f[§9EP§f] §bGroups of player: " + groups));
                    break;
                default:
                    player.sendMessage(
                            Component.text("§f[§9EP§f] §bCommand Help §f(/epv user " + targetName + " group ... <group>)\n" +
                                    "§f[§9EP§f] §9> §aadd\n" +
                                    "§f[§9EP§f] §9> §aaddtimed\n" +
                                    "§f[§9EP§f] §9> §cremove\n" +
                                    "§f[§9EP§f] §9> §aset\n" +
            ...
            switch (action.toLowerCase()) {
                case "add":
                    ...
                    if (!player.hasPermission("elizonperms.user.permission.add")) {
                        player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to add permissions to users."));
                        return;
                    }
                    permPlayer.setMultiStatePermission(args[4].toLowerCase(), MultiState.TRUE);
                    player.sendMessage(Component.text("§f[§9EP§f] §aAdding permission " + args[4] + " to user " + targetName));
                    break;
                case "remove":
                    // Permission check for remove action
                    if (!player.hasPermission("elizonperms.user.permission.remove")) {
                        player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to remove permissions from users."));
                        return;
                    }
                    permPlayer.setMultiStatePermission(args[4].toLowerCase(), null);
                    player.sendMessage(Component.text("§f[§9EP§f] §cRemoving permission " + args[4] + " from user " + targetName));
                    break;
                case "set":
                    // Permission check for set action
                    if (!player.hasPermission("elizonperms.user.permission.set")) {
                        player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to set permissions for users."));
                        return;
                    }
                    //value
                    if(args.length < 6) {
                    ...
                    player.sendMessage(Component.text("§f[§9EP§f] §aSetting permission " + args[4] + " (" + args[5] + ") for user " + targetName));
                    break;
                case "info":
                    // Permission check for info action
                    if (!player.hasPermission("elizonperms.user.permission.info")) {
                        player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to view permission info for users."));
                        return;
                    }
                ...
                default:
                    player.sendMessage(
                            Component.text("§f[§9EP§f] §bCommand Help §f(/epv user " + targetName + " permission ... <string> <true|false|data>)\n" +
                                    "§f[§9EP§f] §9> §aadd\n" +
                                    "§f[§9EP§f] §9> §cremove\n" +
                                    "§f[§9EP§f] §9> §aset\n" +
                                    "§f[§9EP§f] §9> §ainfo"
            ...
            if (!player.hasPermission("elizonperms.user.info")) {
                player.sendMessage(Component.text("§f[§9EP§f] §cYou don't have permission to view general info about users."));
                return;
            }
            List<String> groups = permPlayer.getGroups();
            player.sendMessage(Component.text("§f[§9EP§f] §bGetting general info about user " + targetName));
            player.sendMessage(Component.text("§f[§9EP§f] §bGroups: " + groups));
        } else {
            player.sendMessage(
                    Component.text("§f[§9EP§f] §bCommand Help §f(/epv user " + targetName + " <action>)\n" +
                            "§f[§9EP§f] §9> §apermission\n" +
                            "§f[§9EP§f] §9> §ainfo"
                    )
            );
